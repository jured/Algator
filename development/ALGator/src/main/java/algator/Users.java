...
public class Users {
  ...
  private static String help_msg =
            ...
            + "        who:    user or group (e.g. main.algator or :main.algator)\n"
            ...
            + "     has no owner, commands sets it on user id 1 - main.algator.\n\n"
    ...
    try {
      ...
      if (parts.length==2) { // looking for algorithm
        return getID(String.format(
          "SELECT * from main.algator.entities WHERE name='%s' AND type=2 AND id_parent=%d", parts[1], id_project));
      } else { //looking for testset
        return getID(String.format(
          "SELECT * from main.algator.entities WHERE name='%s' AND type=3 AND id_parent=%d", parts[2], id_project));
      }
    } catch (Exception e) {
      return INVALID_ID;
    }      
  ...
  public static String chmod(String perm, String user, String entity) {   
    try {
      ...
      if (perm.startsWith("+") || perm.startsWith("-")) {
        ...
        id_perm = getID(String.format("SELECT * from main.algator.permissions WHERE permission_code='%s'", perm));
      ...
      String select = String.format(
         "SELECT * FROM main.algator.permissions_%ss WHERE id_%s=%d AND id_entity=%d AND id_permission=%d",
      ...
      if (addPermission) {
        ...
        sql = String.format(
          "INSERT INTO main.algator.permissions_%ss (id_%s,id_entity,id_permission) VALUES (%d,%d,%d)",
          usergroup, usergroup, id_usergroup, id_entity, id_perm
        );         
      } else {
        if (!permExists) return "Permission does not exist.";
        sql = String.format(
          "DELETE FROM main.algator.permissions_%ss WHERE id_%s=%d AND id_entity=%d AND id_permission=%d",
          usergroup, usergroup, id_usergroup, id_entity, id_perm
        );    
      }
  ...
  public static void setowner(String id_owner, String id_entity) {
    ...
    try {
      ...
      String owner_find = String.format(
         "SELECT * FROM main.algator.owners where id_entity=%s",  id_entity
      );
      ...
      if (rst.next()) 
        sql = String.format(
             "UPDATE main.algator.owners SET id_owner=%s WHERE id_entity=%s", id_owner, id_entity
        );
      else
        sql = String.format(
             "INSERT INTO main.algator.owners(id_owner,id_entity) VALUES (%s,%s)", id_owner, id_entity
        );        
