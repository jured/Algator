...
public class Execute {
  ...
  private static void printMsg(Options options) {    
    ...
    formatter.printHelp("main.algator.Execute [options] project_name", options);
  ...
  private static void runAlgorithms(String dataRoot, String projName, String algName,
    ...
    if (!f.exists()) {
      ATGlobal.verboseLevel=1;
      ATLog.log("Project configuration file '" + f.getAbsolutePath() + "' does not exist for " + projName, 1);
    ...
    log.info("here6, algs size: {}", eAlgs.size());
    ...
    if (printOnly) {    
      ...
      for (int i = 0; i < eAlgs.size(); i++) {
	      for (int j = 0; j < eTests.size(); j++) {
	        log.info("here11, Executin something....");
